...
private:
	...
	vector<string> *list(TrieNode *node, vector<string> input, string pattern)
	{
        ...
        for(int i=0;i<26;i++)
        {
        	if(isPresent[i] == true)
        	{
        		...
        		{
        			vector<string> *temp = list(node->children[i],input,pattern);
	...
	void autoComplete(TrieNode*root, vector<string> input, string pattern) 
	{
        ...
        output = list(temp,input,pattern)
